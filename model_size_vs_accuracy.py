...
def run_iid(dataset : str, 
    ...
    if obj == 'single':
        ...
        if classifier == 'ff':
            ff_acc, ff_prec, ff_rec, max_depth, total_leaves = ff(dataframes_to_send, max_depth)
...
if __name__ == "__main__":
    ...
    for _, dset in datasets.iterrows():
        ...
        for obj in obj_list:
            ...
            name, acc, prec, rec, max_depth, total_leaves = main(dset['dataset'], int(dset['total']*dset[obj]))
            out = [dset['dataset'], obj, int(dset['total']*dset[obj]), max_depth, total_leaves, acc]
