...
'''
This program will use the raw datasets downloaded in the root directory and write the condensed datasets using various FS methods into the "./condensed" directory in csv format. Each dataset is passed through the data_preprocessing function that will convert all text values to numeric floats, z-score normalize the data, and convert all Class values to integers starting from 0. 
'''
...
datasets = pd.DataFrame(dataset_list, columns = ['dataset', 'total', 'single', 'multi', 'anova', 'rfe', 'nofs'])
...
def run_iid(
    ...
    if obj == 'single':
        ...
        feature_list = list(reduce(set.intersection, (set(lst) for lst in feature_list)))
        print('feature list: ', feature_list)
        print('number of features: ', len(feature_list))
...
if __name__ == "__main__":
    ...
    for _, dset in datasets.iterrows():     # for each dataset
        ...
        for obj in obj_list:
            ...
            df.to_csv('./datasets/condensed/' + dataset + '_condensed_using_' + obj + '.csv', index=False)
