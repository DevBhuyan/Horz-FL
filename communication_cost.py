...
# TIP: If you run NoFS, please uncomment the below code and comment out the other dataset list
# dataset_list = [
#                 # ['ionosphere', 5, 33, 1],
#                 # ['wdbc', 5, 31, 1],
#                 # ['vowel', 2, 12, 1],
...
# TIP: These are the limited(expected) ranges of optimal feature sets for this task, heuristics to optimize time consumption
dataset_list = [
    ["ionosphere", 16, 20, 1],
    ["wdbc", 5, 12, 1],
    ["vowel", 12, 12, 1],
...
def run_iid(
    ...
    """Driver function to compute optimal communication iterations for the
    FedAvg algorithm for maximum accuracy.
    ...
    if obj == "single":
        ...
        if classifier == "mlp":
            MLP_acc, MLP_prec, MLP_rec = Fed_MLP(
...
if __name__ == "__main__":
    for _, dset in datasets.iterrows():  # for each dataset
        for obj in obj_list:
            ...
            try:
                with open(
                    "_".join([classifier, obj, dset["dataset"], "cache.csv"]), "r"
                ) as f:
                    reader = csv.reader(f)
            ...
            for comm_iter in range(
                (row_count // (dset["ub"] - dset["lb"] + 1) + 5), comm_iters + 1
            ):  # for each comm_iter
                lftr = []
                num_ftr_accuracies = []
                num_ftr_precs = []
                ...
                with open(
                    "_".join([classifier, obj, dset["dataset"], "cache.csv"]),
                    "a",
                    newline="",
                ) as f:
                    writer = csv.writer(f)
            ...
            # The outputs will be written to a file named f"{classifier}_{obj}_{dset["dataset"]}_cache.csv". First col indicated number of iterations
